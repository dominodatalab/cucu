Feature: Run scenarios conditionally
  ...
  Background: Clean up files
    Given I delete the file at "{TMPDIR}/skip_first_scenario.txt" if it exists
      And I delete the file at "{TMPDIR}/skip_second_scenario.txt" if it exists
  ...
  Scenario: User can run scenarios conditionally based on presence of a file
    Given I run the command "cucu run data/features/feature_skipping_scenarios_by_file_presence.feature --results {CUCU_RESULTS_DIR}/skip_scenarios_conditionally_results" and save stdout to "STDOUT" and expect exit code "0"
     ...
     When I create a file at "{TMPDIR}/skip_first_scenario.txt" with the following:
      """
      bananas
      """
     ...
     Then I should see "{STDOUT}" matches the following
      ...
      Feature: Feature with skipping scenarios by file presence
        ...
        Scenario: That runs only when \{TMPDIR\}/skip_first_scenario.txt does not exist
      .* WARNING SKIP Scenario That runs only when \{TMPDIR\}/skip_first_scenario.txt does not exist: skipping scenario since file at "\{filepath\}" exists
          Given I skip this scenario if the file at "\{TMPDIR\}/skip_first_scenario.txt" exists
