Feature: Before retry handlers
  ...
  Scenario: User can use before retry handler handle unexpected UI elements
    Given I create a file at "{CUCU_RESULTS_DIR}/before_retry_handlers/environment.py" with the following:
      ...
      """
     Then I run the command "cucu run {CUCU_RESULTS_DIR}/before_retry_handlers --results {CUCU_RESULTS_DIR}/before_retry_handlers_results --no-color-output --logging-level=debug" and save stdout to "STDOUT", stderr to "STDERR" and expect exit code "0"
