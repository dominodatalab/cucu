Feature: Browser console logs
  ...
  Scenario: User will find per scenario executed a logs directory containing browser logs
    Given I run the command "cucu run data/features/scenario_with_console_logs.feature --results {CUCU_RESULTS_DIR}/console-logging" and save stdout to "STDOUT" and expect exit code "0"
