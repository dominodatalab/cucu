Feature: File downloads
  ...
  Scenario: User can download a file through the browser as regex and store filename
    Given I start a webserver at directory "data/www" and save the port to the variable "PORT"
      And I open a browser at the url "http://{HOST_ADDRESS}:{PORT}/files.html"
     When I click the link "download this file"
     Then I wait to see the downloaded file "file.*"

     When I click the link "download this file"
     Then I wait to see the downloaded regex file "file.*" and save the filename to the variable "FILE_NAME"
