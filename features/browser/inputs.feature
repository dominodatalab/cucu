Feature: Inputs
  ...
  Scenario: User can write into an <input type="date">
    Given I open a browser at the url "http://{HOST_ADDRESS}:40000/inputs.html"
  ...
  Scenario: User can wait to see an input
    Given I set the variable "CUCU_STEP_WAIT_TIMEOUT_S" to "5000"
      And I open a browser at the url "http://{HOST_ADDRESS}:40000/inputs.html?delay_page_load_ms={CUCU_STEP_WAIT_TIMEOUT_S}"
  ...
  Scenario: User can wait up to 10s to write into an input
    Given I open a browser at the url "http://{HOST_ADDRESS}:40000/inputs.html?delay_page_load_ms=10000"
     When I wait to write "01/01/2019" into the input "input type=date"
