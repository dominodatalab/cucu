Feature: Secrets
  ...
  Scenario: User can override cucu_secrets redaction using a cucurc.yml allowlist
    Given I create a file at "{CUCU_RESULTS_DIR}/features_with_secrets_and_allowlist/environment.py" with the following:
      ...
      from cucu.environment import *
      """
      And I create a file at "{CUCU_RESULTS_DIR}/features_with_secrets_and_allowlist/steps/__init__.py" with the following:
      """
      from cucu import run_steps
      from cucu.steps import *
      ...
      """
      And I create a file at "{CUCU_RESULTS_DIR}/features_with_secrets_and_allowlist/features/feature_that_spills_the_beans.feature" with the following:
      """
      Feature: Feature that spills the beans
        ...
        Scenario: This scenario prints some secrets to the logs
         Given I echo "the current user is \{USER\}"
           And I echo "\{MY_SECRET\}"
           And I echo "your home directory is at \{HOME\}"
           And I echo "\{TEST_SECRET\}"
          Then I use a step with substeps that has secrets
      """
     When I run the command "cucu run {CUCU_RESULTS_DIR}/features_with_secrets_and_allowlist --results={CUCU_RESULTS_DIR}/features_with_secrets_and_allowlist_results --generate-report --report={CUCU_RESULTS_DIR}/features_with_secrets_and_allowlist_report" and save stdout to "STDOUT", stderr to "STDERR" and expect exit code "0"
     Then I should see "{STDOUT}" does not contain "super secret"
      And I should see "{STDOUT}" does not contain "another secret"
      And I should see "{STDOUT}" contains "test_secret"
      And I should see the file at "{CUCU_RESULTS_DIR}/features_with_secrets_and_allowlist_results/run.json" contains the following:
      """
      admin_secret
      """
      And I should see the file at "{CUCU_RESULTS_DIR}/features_with_secrets_and_allowlist_report/Feature that spills the beans/This scenario prints some secrets to the logs/index.html" contains the following:
      """
      admin_secret
      """
      And I should see the file at "{CUCU_RESULTS_DIR}/features_with_secrets_and_allowlist_report/Feature that spills the beans/This scenario prints some secrets to the logs/logs/cucu.debug.console.log" contains the following:
      """
      admin_secret
      """
