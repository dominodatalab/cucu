Feature: Run
  ...
  Scenario: User gets an error when running an inexistent feature file
    Given I run the command "cucu run data/features/inexistent.feature" and save stdout to "STDOUT", stderr to "STDERR" and exit code to "EXIT_CODE"
  ...
  Scenario: User gets expected output when running steps with substeps
    Given I run the command "cucu run data/features/scenario_with_substeps.feature" and save stdout to "STDOUT", stderr to "STDERR" and exit code to "EXIT_CODE"
  ...
  Scenario: User gets expected non zero exit code when a scenario fails
    Given I run the command "cucu run data/features/feature_with_failing_scenario.feature" and save stdout to "STDOUT", stderr to "STDERR" and exit code to "EXIT_CODE"
     Then I should see "{EXIT_CODE}" is equal to "1"
      And I should see "{STDOUT}" matches the following
      """
