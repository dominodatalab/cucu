Feature: Report
  ...
  Scenario: User can run a basic non UI test and get an HTML test report
    Given I run the command "cucu run data/features/echo.feature --results {CUCU_RESULTS_DIR}/non-ui-results" and save exit code to "EXIT_CODE"
     Then I should see "{EXIT_CODE}" is equal to "0"
     When I run the command "cucu report {CUCU_RESULTS_DIR}/non-ui-results --output {CUCU_RESULTS_DIR}/non-ui-report" and save exit code to "EXIT_CODE" 
     Then I should see "{EXIT_CODE}" is equal to "0"
