  ...
  Scenario: User can sort by various fields in the HTML test report
    Given I run the command "cucu run data/features/tagged_features --tags @feature2 --show-skips --results {CUCU_RESULTS_DIR}/sorting-in-reports-results" and expect exit code "0"
     ...
     Then I wait to see a table that matches the following:
       | Start at | Features.*             | Scenarios.*                                      | Steps.* | Status  | Duration.* | Replay.* |
       | .*       | First tagged feature   | First scenario within the first tagged feature   | 1       | skipped | .*         | 🔁       |
       | .*       | Second tagged feature  | First scenario within the second tagged feature  | 1       | passed  | .*         | 🔁       |
       | .*       | Third tagged feature   | First scenario within the third tagged feature   | 1       | skipped | .*         | 🔁       |
