...
Feature: Report with custom subheader
  ...
  Scenario: User can register a custom subheader handler for reporting
    Given I create a file at "{CUCU_RESULTS_DIR}/report_with_custom_subheader_handling/environment.py" with the following:
     ...
     Then I should see the text "this is a subheader of Scenario with a custom subheader handler at {CUCU_RESULTS_DIR}/report_with_custom_subheader_handling/feature_with_custom_subheader_handling.feature:3"
  ...
  Scenario: User can register a custom subheader handler for reporting and use "cucu report" after the run
    Given I create a file at "{CUCU_RESULTS_DIR}/report_with_custom_subheader_handling_cmd/environment.py" with the following:
      ...
      from cucu.environment import *
      """
      And I create a file at "{CUCU_RESULTS_DIR}/report_with_custom_subheader_handling_cmd/steps/__init__.py" with the following:
      """
      from cucu.steps import *
      from cucu import register_custom_scenario_subheader_in_report_handling
      ...
      register_custom_scenario_subheader_in_report_handling(subheader_generator)
      """
      And I create a file at "{CUCU_RESULTS_DIR}/report_with_custom_subheader_handling_cmd/feature_with_custom_subheader_handling_cmd.feature" with the following:
      """
      Feature: Feature with custom subheader handling
        ...
        Scenario: Scenario with a custom subheader handler
          Given I echo "nothing to see here"
      """
      And I run the command "cucu run {CUCU_RESULTS_DIR}/report_with_custom_subheader_handling_cmd --results {CUCU_RESULTS_DIR}/custom_subheader_handling_cmd_results" and expect exit code "0"
