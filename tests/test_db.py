...
def test_relationships_and_record_completion(sample_records_combined):
    check.equal(sample_records_combined["worker"].cucu_run_id, "test_run")
    check.equal(
        sample_records_combined["feature"].worker_run_id, "test_worker"
    )
    check.equal(
        sample_records_combined["scenario"].feature_run_id, "test_feature"
    )
    check.equal(
        sample_records_combined["step"].scenario_run_id, "test_scenario"
    )
...
def test_flat_view_functionality(sample_records_combined):
    db.db.execute_sql("""
    ...
    cursor = db.db.execute_sql(
        "SELECT * FROM flat WHERE cucu_run_id = ?", ("test_run",)
    )
    ...
    row = results[0]
    check.equal(row[0], "test_run")
    check.equal(row[1], "2024-01-01T10:00:00")
    check.equal(row[2], 2.5)
    ...
    cursor = db.db.execute_sql(
        "SELECT * FROM flat WHERE cucu_run_id = ?", ("consistency_test",)
    )
    consistency_results = cursor.fetchall()

    check.equal(len(consistency_results), 1)
    consistency_row = consistency_results[0]
    check.equal(consistency_row[0], "consistency_test")
    check.equal(consistency_row[3], "Consistency Feature")
    check.equal(consistency_row[4], "Consistency Scenario")
    check.equal(consistency_row[5], "consistency consistency")
