...
def init_database_in_before_all(ctx):
    """
    Initialize database in before_all hook if database is enabled.
    ...
    if conn:
        ...
        # Create the CucuRun record
        browser = getattr(ctx, "browser_name", "unknown")
        worker_count = CONFIG.get("WORKER_COUNT", 1)
        headless = CONFIG.get("HEADLESS", True)
        results_dir = CONFIG.get("RESULTS_DIR", "results")

        try:
            ctx.database["cucu_run_id"] = create_cucu_run(
                conn,
            ...
            logger.info(
                f"Created CucuRun record with ID {ctx.database['cucu_run_id']}"
            )
        except Exception as e:
            logger.error(f"Failed to create CucuRun record: {str(e)}")


def finalize_database_in_after_all(ctx):
    """
    Finalize database in after_all hook if database is enabled.
    ...
    if ctx.database.get("cucu_run_id"):
        ...
        if conn:
            try:
                ...
                logger.error(f"Failed to update CucuRun record: {str(e)}")


def create_feature_in_before_feature(ctx, feature):
    """
    Create a Feature record in before_feature hook if database is enabled.
    ...
    if ctx.database.get("cucu_run_id"):
        ...
        if conn:
            try:
                ...
                logger.error(f"Failed to create Feature record: {str(e)}")


def update_feature_in_after_feature(ctx, feature):
    """
    Update a Feature record in after_feature hook if database is enabled.
        ...
        if conn:
            try:
                ...
                if hasattr(feature, "start_time") and feature.start_time:
                    total_duration_ms = int(
                        (time.time() - feature.start_time) * 1000
                    )
                ...
                logger.error(f"Failed to update Feature record: {str(e)}")


def create_scenario_in_before_scenario(ctx, scenario):
    """
    Create a Scenario record in before_scenario hook if database is enabled.
    ...
    if ctx.database.get("current_feature_id"):
        ...
        if conn:
            try:
                ...
                logger.error(f"Failed to create Scenario record: {str(e)}")


def update_scenario_in_after_scenario(ctx, scenario):
    """
    Update a Scenario record in after_scenario hook if database is enabled.
        ...
        if conn:
            try:
                ...
                if hasattr(scenario, "start_time") and scenario.start_time:
                    total_duration_ms = int(
                        (time.time() - scenario.start_time) * 1000
                    )
                ...
                logger.error(f"Failed to update Scenario record: {str(e)}")


def create_section_in_section_step(ctx, level, text, order_index=None):
    """
    Create a Section record when a section step is executed.

    Args:
        ctx: The behave context
        level: The section heading level (1-4)
    ...
    if ctx.database.get("current_scenario_id"):
        ...
        if conn:
            try:
                ...
                logger.debug(
                    f"Created Section record with ID {section_id}, level {level}"
                )
                return section_id
            except Exception as e:
                logger.error(f"Failed to create Section record: {str(e)}")
...
def create_step_in_before_step(ctx, step):
    """
    Create a Step record in before_step hook if database is enabled.
    ...
    if ctx.database.get("current_scenario_id"):
        ...
        if conn:
            try:
                ...
                logger.error(
                    f"Failed to create Step/StepRun records: {str(e)}"
                )


def update_step_in_after_step(ctx, step):
    """
    Update a StepRun record in after_step hook if database is enabled.
    ...
    if ctx.database.get("current_step_run_id"):
        ...
        if conn:
            try:
                ...
                logger.error(f"Failed to update StepRun record: {str(e)}")


def record_screenshot_artifact(ctx, filepath, step_run_id=None):
    """
    Record a screenshot as an artifact in the database.

    Args:
        ctx: The behave context
        filepath: Path to the screenshot file
        step_run_id: Optional step run ID
    ...
    if conn:
        try:
            ...
            logger.error(f"Failed to record screenshot artifact: {str(e)}")
