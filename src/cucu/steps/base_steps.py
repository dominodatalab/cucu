...
def expect_the_following_step_to_fail(ctx, message):
    try:
        ...
        if str(exception).find(message) == -1:
            raise RuntimeError(f'expected failure message was "{str(exception)}" not "{message}"')
        return
...
def should_see_previous_step_took_less_than(ctx, seconds):
    if ctx.previous_step_duration > float(seconds):
        raise RuntimeError(f'previous step took {ctx.previous_step_duration}, which is more than {seconds}')
...
def should_see_previous_step_took_more_than(ctx, seconds):
    if ctx.previous_step_duration < float(seconds):
        raise RuntimeError(f'previous step took {ctx.previous_step_duration}, which is less than {seconds}')
