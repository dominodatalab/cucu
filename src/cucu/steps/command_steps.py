...
def runs_command_and_save_everything(context, command, stdout_var, stderr_var, exit_code_var):
    run_command(command, stdout_var=stdout_var, stderr_var=stderr_var, exit_code_var=exit_code_var)


@step('I run the command "{command}" and save stdout to "{stdout_var}", exit code to "{exit_code_var}"')
def run_command_and_save_stdout_and_exit_code(context, command, stdout_var, exit_code_var):
...
@step('I run the following script and save stdout to "{stdout_var}", stderr to "{stderr_var}", exit code to "{exit_code_var}"')
def run_script_and_save_everything(context, stdout_var, stderr_var, exit_code_var):
    run_script(context.text, stdout_var=stdout_var, stderr_var=stderr_var, exit_code_var=exit_code_var)
